...
void GE_pthread_dynamic() {
	...
	pthread_mutex_init(&task_lock, NULL);  //初始化任务锁
	for (int t_id = 0; t_id < NUM_THREADS; t_id++) {//分配任务
		pthread_mutex_lock(&task_lock); //加锁
		param[t_id].t_id = task;
		param[t_id].num = num;
		pthread_create(&handle[t_id], NULL, GE_lock_thread, &param[t_id]);
		task++;
		pthread_mutex_unlock(&task_lock); //解锁
	}
	...
	free(param);
	pthread_mutex_destroy(&lock);
	pthread_mutex_destroy(&task_lock);  //销毁任务锁
}
